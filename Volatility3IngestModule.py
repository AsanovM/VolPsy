...
try:
    # Used to nudge the UI to refresh File View when new files/directories are added
    from org.sleuthkit.autopsy.casemodule.events import ModuleContentEvent as _ModuleContentEvent
except Exception:
    _ModuleContentEvent = None
...
def _normalize_dump_name(name):
    """Return display name for dump artifacts without trailing .img suffix.
    Keep .img when it's a single extension (no second dot before it).
    Path on disk stays intact; only DS display name is adjusted.
    """
    try:
        if name is None:
            return name
        ...
        if low.endswith('.img'):
            last_dot = s.rfind('.')
            earlier_dot = s.rfind('.', 0, last_dot)
            if earlier_dot >= 0:
                return s[:last_dot]
        return s
    except Exception:
...
def _persist_job_settings(settings_obj):
    try:
        data = {
            ...
            'dump_files': bool(getattr(settings_obj, 'getDumpFiles', lambda: False)()),
            'dump_processes': bool(getattr(settings_obj, 'getDumpProcesses', lambda: False)()),
            'selected_plugins': {
...
def _apply_persisted_to_settings(settings_obj, data):
    try:
        ...
        try:
            if 'dump_files' in data:
                settings_obj.setDumpFiles(bool(data.get('dump_files', False)))
            if 'dump_processes' in data:
                settings_obj.setDumpProcesses(bool(data.get('dump_processes', False)))
        except Exception:
            pass
...
class Vol3Runner(object):
    ...
    def run_plugin_args(self, dump_path, plugin_name, renderer, stdout_path, stderr_path, timeout_sec, cancel_check, plugin_args):
        # Same as run_plugin, but allows passing plugin-specific arguments after the plugin name
        cmd = self._build_base_cmd()
        for opt in self.extra_global_opts:
            cmd.append(self._to_unicode(opt))
        ...
        for a in plugin_args:
            try:
                cmd.append(self._to_unicode(a))
            except Exception:
                cmd.append(unicode(a))
        self._append_command_log(u"PLUGIN {0} ({1}) ARGS :: ".format(self._to_unicode(plugin_name), self._to_unicode(renderer)) + u" ".join(cmd))
        return self._execute(cmd, stdout_path, stderr_path, timeout_sec, cancel_check)

    def run_plugin_with_outputdir(self, dump_path, plugin_name, renderer, stdout_path, stderr_path, timeout_sec, cancel_check, output_dir, plugin_args=None):
        # Places global '-o <output_dir>' before the plugin name (required by vol3 CLI)
        cmd = self._build_base_cmd()
        for opt in self.extra_global_opts:
            cmd.append(self._to_unicode(opt))
        if plugin_args is None:
            plugin_args = []
        cmd.extend([u'--renderer', self._to_unicode(renderer), u'-f', self._to_unicode(dump_path), u'-o', self._to_unicode(output_dir), self._to_unicode(plugin_name)])
        for a in plugin_args:
            try:
                cmd.append(self._to_unicode(a))
            except Exception:
                cmd.append(unicode(a))
        self._append_command_log(u"PLUGIN {0} ({1}) OUTDIR :: ".format(self._to_unicode(plugin_name), self._to_unicode(renderer)) + u" ".join(cmd))
        return self._execute(cmd, stdout_path, stderr_path, timeout_sec, cancel_check)
...
class Vol3JobSettings(IngestModuleIngestJobSettings):
    ...
    def __init__(self):
        ...
        self._vol_exe = ""
        self._reports_root = ""
        self._dump_files = False
        self._dump_processes = False
    ...
    # Extraction toggles
    def setDumpFiles(self, value):
        try:
            self._dump_files = bool(value)
        except Exception:
            self._dump_files = False

    def getDumpFiles(self):
        return bool(self._dump_files)

    def setDumpProcesses(self, value):
        try:
            self._dump_processes = bool(value)
        except Exception:
            self._dump_processes = False

    def getDumpProcesses(self):
        return bool(self._dump_processes)
    ...
    def serialize(self):
        payload = {
            ...
            "dump_files": bool(self._dump_files),
            "dump_processes": bool(self._dump_processes)
        }
        try:
    ...
    def deserialize(self, serialized):
        ...
        try:
            ...
            self._dump_files = bool(data.get("dump_files", False))
            self._dump_processes = bool(data.get("dump_processes", False))
...
class Vol3SettingsPanel(IngestModuleIngestJobSettingsPanel):
    ...
    def _build_ui(self):
        ...
        # Extraction toggles (optional dump features)
        opts = JPanel()
        self.dump_files_cb = JCheckBox("Dump files")
        self.dump_files_cb.setToolTipText("Extract files using windows.dumpfiles")
        self.dump_procs_cb = JCheckBox("Dump processes")
        self.dump_procs_cb.setToolTipText("Dump process memory using windows.memdump (PID list from pslist/pstree/json)")
        opts.add(self.dump_files_cb)
        opts.add(self.dump_procs_cb)
        opts.setBorder(TitledBorder("Extraction options"))

        north_container = JPanel()
        north_container.setLayout(BoxLayout(north_container, BoxLayout.Y_AXIS))
        north_container.add(paths_panel)
        north_container.add(opts)
        self.add(north_container, BorderLayout.NORTH)
    ...
    def _load_settings(self, settings):
        ...
        # Set extraction toggles from settings (default false)
        try:
            self.dump_files_cb.setSelected(bool(settings.getDumpFiles()))
        except Exception:
            self.dump_files_cb.setSelected(False)
        try:
            self.dump_procs_cb.setSelected(bool(settings.getDumpProcesses()))
        except Exception:
            self.dump_procs_cb.setSelected(False)
    ...
    def getSettings(self):
        ...
        # Dump toggles
        try:
            settings.setDumpFiles(self.dump_files_cb.isSelected())
        except Exception:
            pass
        try:
            settings.setDumpProcesses(self.dump_procs_cb.isSelected())
        except Exception:
            pass
...
class Vol3DataSourceIngestModule(DataSourceIngestModule):
    ...
    def _fire_content_added(self, af):
        try:
            if af is None:
                return
            if _ModuleContentEvent is None:
                return
            try:
                IngestServices.getInstance().fireModuleContentEvent(_ModuleContentEvent(af))
            except Exception:
                pass
        except Exception:
            pass

    def _progress_update(self, progressBar, step, label=None):
        # Try several API shapes to maximize compatibility across Autopsy versions:
        #  - progress(String, int)
        #  - progress(int, String)
        #  - progress(String)
        #  - progress(int)
        try:
            if label is not None and len(label) > 0:
                try:
                    progressBar.progress(label, step)
                    return
                except Exception:
                    pass
                try:
                    progressBar.progress(step, label)
                    return
                except Exception:
                    pass
                try:
    ...
    # ----- Dump helpers -----
    def _collect_pids_from_json(self):
        pids = set()
        try:
            if self.json_dir is None or not self.json_dir.exists():
                return []
            files = self.json_dir.listFiles()
            if files is None:
                return []
            for f in files:
                try:
                    name = f.getName()
                    if name is None or not name.lower().endswith('.jsonl'):
                        continue
                    low = name.lower()
                    if not (low.startswith('windows.pslist') or low.startswith('windows.psscan')):
                        continue
                    # Parse JSON lines and collect pid keys
                    br = BufferedReader(InputStreamReader(FileInputStream(f), 'UTF-8'))
                    try:
                        line = br.readLine()
                        count = 0
                        while line is not None and count < 200000:
                            try:
                                obj = json.loads(line)
    ...
    def _attach_dump_outputs(self, dataSource, dir_path, subdir_name=None):
        try:
            if dir_path is None:
                return 0
            root = File(dir_path)
            ...
            parent = None
            try:
                if subdir_name is not None and len(unicode(subdir_name)) > 0:
                    parent = self._get_or_create_vol_subdir(dataSource, subdir_name)
            except Exception:
                parent = None
            if parent is None:
            ...
            added = 0
            # Prepare a DerivedFiles path for last-resort registration
            try:
                caseDir = Case.getCurrentCase().getCaseDirectory()
                derivedRoot = File(File(caseDir, "DerivedFiles"), "Volatility3")
                if not derivedRoot.exists():
                    derivedRoot.mkdirs()
                derivedDumps = File(derivedRoot, "dumps")
                if not derivedDumps.exists():
                    derivedDumps.mkdirs()
            except Exception:
                derivedRoot = None
                derivedDumps = None
            ...
            while len(stack) > 0:
                ...
                try:
                    name = d.getName()
                    size = d.length()
                    ts = long(d.lastModified() / 1000) if d.lastModified() > 0 else 0
                    path = d.getAbsolutePath()
                    # Prefer pretty display name without trailing .img
                    base_disp = _normalize_dump_name(name)
                    candidate = base_disp
                    lf = None
                    # Avoid duplicates by suffixing if needed
                    for i in range(0, 5):
                        nm = candidate if i == 0 else (base_disp + '_' + str(i))
                        try:
                            lf = self._add_local_file_robust(sk, parent, nm, size, ts, path, 'application/octet-stream')
                            if lf is not None:
                                added += 1
                                try:
                                    self._fire_content_added(lf)
                                except Exception:
                                    pass
                                break
                        except Exception:
                            continue
                    # Fallback: if LocalFile registration failed for all attempts,
                    # copy the file into Case/DerivedFiles/Volatility3/dumps and register as Derived under DS
                    try:
                        if lf is None and derivedDumps is not None:
                            # Build a unique destination name
                            destFile = File(derivedDumps, name)
                            idx2 = 0
                            while destFile.exists() and idx2 < 100:
                                idx2 += 1
                                destFile = File(derivedDumps, name + "_" + str(idx2))
                            # Copy bytes
                            inSt = None
                            outSt = None
                            try:
                                inSt = FileInputStream(d)
                                ...
                                try:
                                    ...
                                    pass
                            # Compute relative path to case dir
                            try:
                                ...
                                localPathRel = destFile.getName()
                            details = u"Volatility dump artifact"
                            try:
                                # Use legacy overload to avoid enum issues across versions
                                derived = sk.addDerivedFile(_normalize_dump_name(destFile.getName()), localPathRel, destFile.length(),
                                                            ts, ts, ts, ts, True, parent,
                                                            details, MODULE_NAME, MODULE_VERSION, "")
                                if derived is not None:
                                    added += 1
                                    try:
                                        self._fire_content_added(derived)
                                    except Exception:
                                        pass
                            except Exception:
                                pass
                    ...
                    continue
            try:
                # Nudge UI to refresh the Volatility3 (or sub) directory under DS
                self._fire_content_added(parent)
            except Exception:
                pass
            return added
        except Exception:
    ...
    def _attach_dump_outputs_retry(self, dataSource, dir_path, subdir_name=None, rounds=5, sleep_ms=200):
        try:
            total = 0
            seen_growth = 0
            for i in range(0, max(1, int(rounds))):
                added = 0
                try:
                    added = int(self._attach_dump_outputs(dataSource, dir_path, subdir_name))
                except Exception:
                    added = 0
                total += max(0, added)
                if added > 0:
                    seen_growth += 1
                # Short-circuit if no further growth across last two rounds
                if i >= 1 and added == 0 and seen_growth == 0:
                    break
                # Small pause to catch late-arriving files
                try:
                    if i < rounds - 1 and sleep_ms > 0:
    ...
    def startUp(self, context):
        ...
        self.register_mode = 'both'
        try:
            _safe_debug_log(u"[ingest] attach_as_ds setting: {0}".format(str(self.attach_as_ds).lower()))
        except Exception:
            pass
        try:
            _safe_debug_log(u"[ingest] register_mode: {0}".format(self.register_mode))
        except Exception:
            pass
        ...
        # Probe available plugins once so we can gracefully skip unsupported dump features
        self.has_dumpfiles = False
        self.has_memdump = False
        try:
            qres, qout, qerr = self.runner.query_available_plugins(min(120, self.timeout_sec))
            names = []
            if qout is not None:
                for line in qout.splitlines():
                    ...
                    if base not in names:
                        names.append(base)
            self.has_dumpfiles = ("windows.dumpfiles" in names)
            self.has_memdump = ("windows.memdump" in names)
        except Exception:
            self.has_dumpfiles = False
            self.has_memdump = False
    ...
    def process(self, dataSource, progressBar):
        ...
        # Ensure a visible 'Volatility3' directory exists under the current DS before any registrations
        try:
            self._get_or_create_vol_dir(dataSource)
        except Exception:
            pass
        ...
        current_step = 0
        self._progress_update(progressBar, current_step, u"Preparing Volatility 3 run")
        self.success_count = 0
        self.failure_count = 0
        self.timeout_count = 0
        ...
        for plugin_name in plugins:
            ...
            self._progress_update(progressBar, current_step, u"Running {0} (jsonl)".format(plugin_name))
            ...
            self._record_error(plugin_name, "jsonl", stderr_json_tmp, json_result)
            current_step += 1
            self._progress_update(progressBar, current_step, u"Finished {0} (jsonl)".format(plugin_name))
            ...
            self._progress_update(progressBar, current_step, u"Running {0} (pretty)".format(plugin_name))
            ...
            self._record_error(plugin_name, "pretty", stderr_txt_tmp, txt_result)
            current_step += 1
            self._progress_update(progressBar, current_step, u"Finished {0} (pretty)".format(plugin_name))
            ...
            if not getattr(self, 'attach_as_ds', True):
                try:
                    ...
                    pass
                # 1) Всегда создаём/обновляем Derived-копию (Documents/DerivedData)
                try:
                    _df = self._create_derived_file(dataSource, plugin_name, txt_path)
                except Exception:
                    _df = None
                # 2) Всегда добавляем LocalFile под DS/Volatility3 (видимость в дереве DS)
                try:
        ...
        # Optional extraction: dump files/process memory (Windows only)
        # Implemented with simple, Jython-friendly control flow.
        try:
            want_files = False
            want_procs = False
            try:
                if hasattr(self.settings, 'getDumpFiles'):
                    want_files = bool(self.settings.getDumpFiles())
            except Exception:
                want_files = False
            try:
                if hasattr(self.settings, 'getDumpProcesses'):
                    want_procs = bool(self.settings.getDumpProcesses())
            except Exception:
                want_procs = False

            if want_files or want_procs:
                dumps_root = File(self.report_root_dir, 'dumps')
                try:
                    dumps_root.mkdirs()
                except Exception:
                    pass

                os_is_windows = (self.last_detected_os == OS_WINDOWS)

                # Windows-specific actions
                if os_is_windows:
                    if want_files:
                        # Gracefully skip if this build of vol3 doesn't have dumpfiles
                        if not getattr(self, 'has_dumpfiles', False):
                            try:
                                IngestServices.getInstance().postMessage(IngestMessage.createMessage(IngestMessage.MessageType.INFO, MODULE_NAME,
                                    'windows.dumpfiles plugin not available in this Volatility build; skipping'))
                            except Exception:
                                pass
                        else:
                            try:
                                files_dir = File(dumps_root, 'files')
                                files_dir.mkdirs()
                                out_log = File(self.logs_dir, 'dumpfiles.stdout.txt').getAbsolutePath()
                                err_tmp = File(self.logs_dir, 'dumpfiles.stderr.tmp').getAbsolutePath()
                                self._progress_update(progressBar, current_step, u'Dumping files (windows.dumpfiles)')
                                res = self.runner.run_plugin_with_outputdir(
                                    dump_path,
                                    'windows.dumpfiles',
                                    'none',
                                    out_log,
                                ...
                                self._record_error('windows.dumpfiles', 'none', err_tmp, res)
                                try:
                                    c = self._attach_dump_outputs_retry(dataSource, files_dir.getAbsolutePath(), u'Dump Files', 5, 200)
                                    if c > 0:
                                        IngestServices.getInstance().postMessage(
                                            IngestMessage.createMessage(
                                                IngestMessage.MessageType.INFO,
                                                MODULE_NAME,
                                                u'Attached {0} dumped files'.format(c)
                                            )
                                        )
                                except Exception:
                                    pass
                    ...
                    if want_procs:
                        # Используем windows.memmap --dump как метод дампа процессов
                        if not getattr(self, 'has_memmap', False):
                            try:
                                IngestServices.getInstance().postMessage(IngestMessage.createMessage(IngestMessage.MessageType.INFO, MODULE_NAME,
                                    'windows.memmap plugin not available in this Volatility build; skipping process dumps'))
                            except Exception:
                                pass
                        else:
                            try:
                                procs_dir = File(dumps_root, 'procs')
                                procs_dir.mkdirs()
                                pids = None
                                try:
                                    pids = self._collect_pids_from_json()
                                except Exception:
                                    pids = None
                                if pids is None:
                                    pids = []
                                ...
                                for pid in pids:
                                    if idx >= max_count:
                                        break
                                    idx += 1
                                    out_log = File(self.logs_dir, 'memmap_{0}.stdout.txt'.format(pid)).getAbsolutePath()
                                    err_tmp = File(self.logs_dir, 'memmap_{0}.stderr.tmp'.format(pid)).getAbsolutePath()
                                    self._progress_update(progressBar, current_step, u'Dumping process PID {0} via memmap'.format(pid))
                                    res = self.runner.run_plugin_with_outputdir(
                                        dump_path,
                                        'windows.memmap',
                                        'none',
                                        ...
                                        self.timeout_sec,
                                        self._is_cancelled,
                                        procs_dir.getAbsolutePath(),
                                        ['--pid', unicode(pid), '--dump']
                                    )
                                self._record_error('windows.memmap(pid={0})'.format(pid), 'none', err_tmp, res)
                                try:
                                    c2 = self._attach_dump_outputs_retry(dataSource, procs_dir.getAbsolutePath(), u'Process Dumps', 3, 150)
                                    if c2 > 0:
                                        IngestServices.getInstance().postMessage(
                                            IngestMessage.createMessage(
                                                IngestMessage.MessageType.INFO,
                                                MODULE_NAME,
                                                u'Attached {0} process memory dumps (memmap)'.format(c2)
                                            )
                                        )
                                except Exception:
                                    pass
                ...
                # Final safety sweep: if dump directories exist, ensure everything is registered under DS
                try:
                    if (want_files or want_procs) and dumps_root is not None:
                        for sub in ('files', 'procs'):
                            try:
                                ddir = File(dumps_root, sub)
                                if ddir is not None and ddir.exists():
                                    try:
                                        subname = u'Dump Files' if sub == 'files' else u'Process Dumps'
                                        c_final = self._attach_dump_outputs_retry(dataSource, ddir.getAbsolutePath(), subname, 3, 150)
                                        try:
                                            if c_final > 0:
                                                IngestServices.getInstance().postMessage(
                                                    IngestMessage.createMessage(
                                                        IngestMessage.MessageType.INFO,
                                                        MODULE_NAME,
                                                        u'Final sweep attached {0} items from dumps/{1}'.format(c_final, sub)
                                                    )
                                                )
                                        except Exception:
                                            pass
                ...
                # Non-Windows: notify user once (features are Windows-only)
                    ...
                    try:
                        IngestServices.getInstance().postMessage(
                            IngestMessage.createMessage(
                                IngestMessage.MessageType.INFO,
                                MODULE_NAME,
                                'Dump features are currently implemented for Windows only'
                            )
                        )
                    except Exception:
                        pass
        ...
        if getattr(self, 'attach_as_ds', True):
            ...
            try:
                ...
                pass
        self._progress_update(progressBar, total_steps, u"Volatility 3 completed")
        ...
        try:
            ...
            pass
        # Release command log path to avoid file handle pinning between runs
        try:
            if getattr(self, 'runner', None) is not None:
                self.runner.set_command_log_path(None)
        except Exception:
            pass
        return DataSourceIngestModule.ProcessResult.OK
    ...
    def _locate_and_export_image(self, dataSource):
        ...
        try:
            ...
            if not tmpDir.exists():
                tmpDir.mkdirs()
            # Preserve original extension if present to avoid double extensions like .mem.img
            try:
                orig = best.getName()
                dot = orig.rfind('.')
                ext = orig[dot:] if (dot > 0 and dot < len(orig) - 1 and dot >= (len(orig) - 8)) else ""
            except Exception:
                ext = ""
            fname = "vol3_image_" + unicode(System.currentTimeMillis()) + ext
            ...
            if derived is None:
                ...
                try:
                    ...
                    try:
                        ...
                        present = False
                    # Consider 'present' only if it is under DS/Volatility3, not under Case/DerivedFiles
                    try:
                        if present and p is not None and 'derivedfiles' in p.lower():
                            present = False
                    except Exception:
                        pass
                    # treat files only in DerivedFiles as not present under DS
                    try:
                        if present and p is not None and 'derivedfiles' in p.lower():
                            present = False
                    except Exception:
                    ...
                    pass
                # If mode is 'both' OR 'local', also add an explicit LocalFile entry regardless of previous checks
                try:
                    if getattr(self, 'register_mode', 'derived') in ('both', 'local'):
                        try:
    ...
    def _get_or_create_vol_dir(self, dataSource):
        ...
        # Prefer a virtual directory so LocalFile children are always visible under DS.
        # Cache per data source id.
        try:
            ds_id = None
            try:
            ...
            # Try to find an existing folder named 'Volatility3'
            try:
                fm = Case.getCurrentCase().getServices().getFileManager()
                ...
                pass
            sk = Case.getCurrentCase().getSleuthkitCase()
            # First choice: Virtual directory under DS
            try:
                ...
                pass
            # Fallbacks: try derived dir (new enums, then legacy)
            try:
    ...
    def _get_or_create_vol_subdir(self, dataSource, subname):
        # Returns a child directory under 'Volatility3' with the given name
        try:
            base = self._get_or_create_vol_dir(dataSource)
            if base is None:
                return None
            # Try to find an existing child with this name
            try:
                fm = Case.getCurrentCase().getServices().getFileManager()
                found = fm.findFiles(dataSource, subname, "Volatility3")
                if found is not None:
                    for af in found:
                        try:
                            if af.isDir() and af.getName() == subname:
                                return af
                        except Exception:
                            continue
            except Exception:
                pass
            # Create via derived directory (new enums first, then legacy)
            try:
                sk = Case.getCurrentCase().getSleuthkitCase()
                ts = long(System.currentTimeMillis() / 1000)
                try:
                    d = sk.addDerivedFile(subname, "", 0,
                                           ts, ts, ts, ts, False, base,
                                           u"Volatility 3 output subdir", MODULE_NAME, MODULE_VERSION, "",
                                           TskData.TSK_FS_NAME_TYPE_ENUM.DIR, TskData.TSK_FS_META_TYPE_ENUM.DIR,
                                           None, 0, None, SleuthkitCase.EncodingType.NONE)
                    if d is not None:
                        return d
